...
- `docs/metric-text-layer.md` — the screen-reader-only sentences that give every published number its scope, window and as-of date, and the rules they follow (full numbers, no invented dates, three health states, label echo, every selector state). Read before touching `common-util/metric-context.ts`, `components/ui/MetricContext.tsx`, any `sr-only` metric mirror, or `components/ui/popover.tsx`.
- `docs/structured-data.md` — the JSON-LD blocks (Organization, FAQPage, Article, DataCatalog/Dataset) and the rule that each is built from the data the page renders, never a second copy. Read before touching `common-util/structured-data.ts`, `components/JsonLd.tsx`, `components/DataPage/datasets.ts`, or adding a section to `/data`.
- `docs/chart-data-integrity.md` — the paging rule every chart query follows, an inventory of which chart is bounded how, and the known fragilities (wei through `Number`, unguarded circulating supply, dead queries). Read before adding a chart or changing any query behind one.
- `docs/staking-emissions-chart.md` — the four staking lines on `/olas-token` (minted, dispensed, claimable, claimed), what each gap between them means, and why claimed has a slow fallback path. Read before changing `common-util/api/other-metrics/staking-emissions.ts`, `EmissionsToOperators`, or `STAKING_SUBGRAPHS_WITH_CLAIMED_TOTALS`.
...
10. **Indexer URLs live in one place**: every per-chain subgraph and squid endpoint is a `{ chain: url }` map in `common-util/indexers.ts`, split into `*_SUBGRAPH_URLS` (The Graph) and `*_SQUID_URLS` (SQD, OpenReader dialect). `graphql/client.ts` builds each client map from its URL map (`toClients`), the typed chain keys (`REGISTRY_CHAIN_KEYS`, `MARKETPLACE_CHAIN_KEYS`, `MECH_FEES_CHAIN_KEYS` — iterate these to cover every chain; the scope sentences use them too) are read off the maps, and `/data` links render them with `components/DataPage/IndexerLinks`. For a source with both kinds, go through `requestRegistry` / `requestMarketplace` / `requestMechFees` in `common-util/graphql/indexer-requests.ts` with both a subgraph and an OpenReader query. Never derive a source's chains from `CHAIN_LAG_CONFIG` — that is the RPC/lag table, and adding a chain there must not widen any source's scope.
