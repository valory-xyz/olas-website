...
- When aggregating USD values **across pools/chains with different token decimals**
  (e.g. USDC=6, WETH=18) — not for single-chain calcs — **sanity-clamp the result
  before publishing**. A decimals mismatch silently produces huge values rather
  than errors. Return `null` with a fetchError on breach so `mergeWithFallback`
  preserves the previous valid snapshot. See
  `common-util/api/other-metrics/protocol.ts` for the pattern.
