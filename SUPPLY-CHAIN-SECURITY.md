...
**Secret scanning.** [`.github/workflows/secret-scan.yml`](./.github/workflows/secret-scan.yml) runs [`gitleaks`](https://github.com/gitleaks/gitleaks) (SHA-pinned action) on every push and PR with full history (`fetch-depth: 0`) so accidentally committed credentials are caught before merge. False positives (test fixtures, example tokens) are suppressed via a root `.gitleaks.toml`. It is a separate workflow, so it is its own required status check in branch protection rather than a `needs:` of `all-checks-passed` (the aggregator can only depend on jobs in the same workflow).
...
- [x] Add `gitleaks` secret scanning ([`.github/workflows/secret-scan.yml`](./.github/workflows/secret-scan.yml), SHA-pinned).
