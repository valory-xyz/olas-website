...
Run the audit gate on every PR via `yarn audit:prod` ([`scripts/audit.mjs`](./scripts/audit.mjs)), invoked by the `audit` job in [.github/workflows/main.yml](./.github/workflows/main.yml). The wrapper runs `yarn audit --groups dependencies`, enforces high/critical gating via exit-code bitmask rather than `--level` (see the "Yarn 1.x audit quirk" note below), and suppresses only the advisory IDs listed in [`.supply-chain/audit-allowlist.json`](./.supply-chain/audit-allowlist.json) — every suppression carries a `reason`, an `added` date, and a `review` date (entries past their review date emit a CI warning). The allowlist currently has **zero** entries; the file is kept even when empty as the running assertion that the gate exists and there is no current debt. `--groups dependencies` restricts the audit to the production tree — `devDependencies` (ESLint / Babel / TypeScript / types) generate substantial transitive-advisory noise and do not ship to users, so they are excluded by policy.
...
**Install-hook diff gate.** The `install-hooks` job runs `yarn install --frozen-lockfile --ignore-scripts` and then `yarn audit:install-hooks` ([`scripts/audit-install-hooks.mjs`](./scripts/audit-install-hooks.mjs)), which fails if any installed package declares a `preinstall`/`install`/`postinstall` script that is not in [`.supply-chain/install-hooks.allowlist`](./.supply-chain/install-hooks.allowlist). This catches a class of change `yarn audit` never sees: a transitive dependency that *starts* running an install script in a new version (not a published CVE). When a legitimate new hook appears, review it and run `yarn audit:install-hooks:update` to record it with a justification. Also blocking (in `all-checks-passed.needs`).
