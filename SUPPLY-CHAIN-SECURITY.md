...
Run the audit gate on every PR via `yarn audit:prod` ([`scripts/audit.mjs`](./scripts/audit.mjs)), invoked by the `audit` job in [.github/workflows/main.yml](./.github/workflows/main.yml). The wrapper runs `yarn audit --groups dependencies --json` and parses the JSON advisory stream directly (Yarn 1.x has no native suppression and its exit code is an unhelpful severity bitmask — see the "Yarn 1.x audit quirk" note below), failing on any high/critical advisory in the production tree that is not listed in [`.supply-chain/audit-allowlist.json`](./.supply-chain/audit-allowlist.json). The allowlist is **schema-validated at load** — a malformed entry (non-numeric `id`, empty `reason`, `review` not `YYYY-MM-DD`) fails the gate with exit 2 rather than silently suppressing. Every suppression carries a `reason`, an `added` date, and a `review` date; the gate emits a CI warning for entries that are **expired** (past their review date) or **stale** (no longer suppressing a current advisory — drift). The allowlist currently has **zero** entries; the file is kept even when empty as the running assertion that the gate exists and there is no current debt. The script mirrors `valory-xyz/autonolas-frontend-mono/scripts/audit.mjs` for fleet parity. `--groups dependencies` restricts the audit to the production tree — `devDependencies` (ESLint / Babel / TypeScript / types) generate substantial transitive-advisory noise and do not ship to users, so they are excluded by policy.
...
**Yarn 1.x audit quirk.** This repo uses Yarn `1.22.22`. Yarn 1.x `yarn audit` exits with a severity bitmask (`1`=info, `2`=low, `4`=moderate, `8`=high, `16`=critical) rather than a threshold comparison against `--level`, so `--level high` filters the *printed* output but does not affect the exit code. [`scripts/audit.mjs`](./scripts/audit.mjs) parses the NDJSON advisory stream directly and fails only on high/critical findings, so the bitmask handling lives in one tested place rather than inline shell. Revisit on a future Yarn Berry migration, which ships `yarn npm audit` with proper severity gating and makes the bitmask dance unnecessary.

The audit job is **blocking**: it is included in `all-checks-passed.needs` and runs without `continue-on-error`, so any high or critical advisory that lands in the production tree will fail PRs until it is addressed (either via a dependency bump that clears the advisory, or via an exact-pinned Yarn `resolutions` entry per [§1](#1-exact-version-pinning-in-packagejson), or — as a last resort when upstream has no fix — an [`.supply-chain/audit-allowlist.json`](./.supply-chain/audit-allowlist.json) entry with a reason and review date).

**Install-hook diff gate.** The `install-hooks` job runs `yarn install --frozen-lockfile --ignore-scripts` and then `yarn audit:install-hooks` ([`scripts/audit-install-hooks.mjs`](./scripts/audit-install-hooks.mjs)), which fails if any installed package declares a non-trivial `preinstall`/`install`/`postinstall` script that is not in [`.supply-chain/install-hooks.allowlist`](./.supply-chain/install-hooks.allowlist). This catches a class of change `yarn audit` never sees: a transitive dependency that *starts* running an install script in a new version (not a published CVE). The scan **recurses into nested `node_modules`** (a hook in a deduped nested copy is not invisible), skips **trivial** no-op hooks (`echo`/`true`/`:`/`exit 0`, with the `echo` filter hardened against shell-metachar chaining), and is **bidirectional** — a stale allowlist entry whose package left the tree also fails the gate, so the list can't quietly rot. When a legitimate new hook appears, review it and run `yarn audit:install-hooks:update` (which preserves existing justification comments and tags only new packages `TODO: justify`). Also blocking (in `all-checks-passed.needs`). Mirrors `valory-xyz/autonolas-frontend-mono/scripts/audit-install-hooks.mjs`.

**Secret scanning.** [`.github/workflows/secret-scan.yml`](./.github/workflows/secret-scan.yml) runs [`gitleaks`](https://github.com/gitleaks/gitleaks) on every push and PR, scanning the checked-out tree so accidentally committed credentials are caught before merge. We run the open-source gitleaks **CLI** (pinned by version + sha256), not the `gitleaks-action` wrapper — the wrapper requires a paid license on GitHub org repos, whereas the binary does not. False positives (test fixtures, example tokens) can be suppressed via a root `.gitleaks.toml`. It is a separate workflow, so it is its own required status check in branch protection rather than a `needs:` of `all-checks-passed` (the aggregator can only depend on jobs in the same workflow).

### 6. Avoid postinstall-heavy dependencies

**Install scripts are skipped by default.** [`.yarnrc`](./.yarnrc) sets `--install.ignore-scripts true`, so `yarn install` (locally, in CI, and on Vercel) does not run any package lifecycle scripts. The repo's runtime native dependencies (`sharp` for `next/image`, `unrs-resolver` for the ESLint import resolver, `bufferutil` for `ws`) all ship prebuilt platform binaries as optional packages, so the build and lint succeed without their install hooks — this was verified with a clean `node_modules` reinstall. A package that genuinely needs its own postinstall to build locally can be installed once with `yarn install --ignore-scripts false` (narrow exception, not the default). New hooks are surfaced and reviewed via the install-hook diff gate ([§5](#5-audit-in-ci)).
...
- [x] Move the audit gate to a wrapper script ([`scripts/audit.mjs`](./scripts/audit.mjs)) backed by [`.supply-chain/audit-allowlist.json`](./.supply-chain/audit-allowlist.json) (empty); add the install-hook diff gate ([`scripts/audit-install-hooks.mjs`](./scripts/audit-install-hooks.mjs) + [`.supply-chain/install-hooks.allowlist`](./.supply-chain/install-hooks.allowlist)) and skip install scripts by default ([`.yarnrc`](./.yarnrc)).
- [x] Add `gitleaks` secret scanning ([`.github/workflows/secret-scan.yml`](./.github/workflows/secret-scan.yml)) — pinned, checksum-verified CLI (license-free on org repos).
- [x] Pin Node via [`.nvmrc`](./.nvmrc) (`22.22.3`) and read it in CI via `node-version-file`.
- [ ] **Fill in real team handles in [`.github/CODEOWNERS`](./.github/CODEOWNERS)** and enable "Require review from Code Owners" in branch protection — the placeholder `@valory-xyz/REPLACE-*` handles route nothing until replaced.
- [ ] **Add `gitleaks` as a required status check** in branch protection (it is a separate workflow, so it is not covered by the `all-checks-passed` aggregator).
- [ ] **Sandbox-test each gate's failure mode** on a throwaway branch (vulnerable dep → audit fails; new postinstall → install-hooks fails; fake secret → gitleaks fails) before relying on them.
