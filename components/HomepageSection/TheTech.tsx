...
const innovations = [
  {
    ...
    description: (
      <>
        The multi-chain protocol coordinates, incentivizes and guides its participants towards
        Olas&apos; goals. Anyone can <Link href="/olas-token#choose-your-role">get involved</Link>{' '}
        through a role-based system and and be rewarded for their useful contributions.
      </>
    ),
...
export const TheTech = ({ hideLearnMoreButton = false }) => (
  <SectionWrapper
    ...
    id="tech"
  >
    <SectionHeading size="max-sm:text-5xl" color="text-gray-900" weight="font-bold">
      Underpinned By Two Core Innovations
    </SectionHeading>

    <div className="grid md:grid-cols-2 gap-x-10 gap-y-4 max-w-5xl mx-auto mb-16">
      {innovations.map((item) => (
        <Card className="flex flex-col gap-6 p-6 bg-white rounded-2xl border-0" key={item.title}>
          <Image
            alt={item.title}
            src={item.image.src}
          ...
          <div className="text-start">
            ...
            {item.descriptionSubText && <p className="mb-2">{item.descriptionSubText}</p>}
