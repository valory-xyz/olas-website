...
} from 'components/HomepageSection/Flywheel/constants';
  ...
  const feeFlowSummary =
    [
      ...
      feeSentence(
        metrics?.protocolFees,
        (a) =>
          `Fees collected ${a} is the marketplace fee taken out of task payments, not revenue additional to them. It covers ${MARKETPLACE_FEE_TOKEN_SCOPE}; it excludes ${MARKETPLACE_FEE_EXCLUDED_SCOPE}, and counts only since the fee went live on ${feeLiveSince}.`
      ),
    ]
      .filter(Boolean)
      .join(' ') || null;
