...
type EmissionsToBondersProps = {
  emissions: EmissionData[];
  loading: boolean;
};

export const EmissionsToBonders = memo(({ emissions, loading }: EmissionsToBondersProps) => {
  const totalBondsClaimable = getCumulativeEmissions(emissions, 'totalBondsClaimable');
  const totalBondsClaimed = getCumulativeEmissions(emissions, 'totalBondsClaimed');
  ...
  return (
    <div className="flex flex-col flex-auto p-4">
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-1 mb-6">
        <LegendItem color={EMISSIONS_CHART_COLORS.available.legend} label="OLAS bonds claimable" />
        <LegendItem color={EMISSIONS_CHART_COLORS.bonds.legend} label="OLAS bonds claimed" />
      </div>
        ...
        <div className="flex-auto h-72">
          {loading ? (
            ...
            <Line
              ...
              options={getEmissionsChartOptions([...totalBondsClaimable, ...totalBondsClaimed])}
            />
          )}
        </div>
      </div>
