...
import { OLAS_ECONOMY_DASHBOARD_URL, PREDICTION_ECONOMY_DASHBOARD_URL } from 'common-util/constants';
...
export const OperateMetrics = () => {
  const { data: metrics } = usePersistentSWR('operateActivityMetrics', fetchMetrics);
    ...
    () => [
      {
        ...
        labelText: 'Operators',
        subText: 'All-time unique participants',
  ...
  return (
    <SectionWrapper customClasses='mt-16'>
      <Card className='flex flex-row gap-6 p-8 mx-auto border border-purple-200 rounded-full text-xl w-fit rounded-2xl bg-gradient-to-t from-[#F1DBFF] to-[#FDFAFF] items-center'>
        {data.map((item, index) => {
          let borderClassName = '';
          if (index == 0) borderClassName += 'lg:border-r-1.5 border-purple-200';
          ...
          return (
            <div key={item.id} className={`text-center w-[345px] py-6 2xl:py-3 px-8 border-gray-300 h-full max-sm:w-full ${borderClassName}`}>
              <div className='flex gap-2 mb-3 justify-center'>
                <Image alt='Operate' src={`/images/operate-page/${item.imageSrc}`} width={35} height={35} />
                <span className='text-lg text-black'>{item.labelText}</span>
              </div>
